...
本目录中的 7 类随机 optimisation instance generators 已完成实例生成、OSQP 求解和 CVXPY 模型一致性验证。依赖统一安装在**仓库根目录的 `.venv`**。历史实测环境和结果见 [复现报告](GENERATORS_REPRODUCTION.md)；本文按当前源码说明接口，其中 Control、Eq QP、Lasso 和 Huber 已支持自定义生成参数。
...
Control 的第二个位置参数现在是 `nu`，Eq QP 和 Huber 的第二个位置参数是 `m`，请通过关键字指定 `seed`；Lasso 的第二个位置参数仍是 `seed`。原来的 `ControlExample(10, seed=1)` 应改为 `ControlExample(nx=10, nu=5, seed=1)`；旧的 `HuberExample(n, seed=...)` 也需要补齐参数，见第 4、5.9 节。
...
Eq QP、Lasso 和 Huber 当前使用 `np.random.default_rng(seed)` 局部 RNG，不改变 NumPy 全局随机状态。Control、Random QP、Portfolio 和 SVM 仍使用 `np.random.seed(seed)`，会改变全局随机状态，不宜在线程中并发调用这些依赖全局 RNG 的构造函数。同一固定依赖环境、全部构造参数和 seed 可复现同一实例；局部 RNG 与旧全局 RNG 的数值序列不同，也不应假设跨 NumPy/SciPy 版本仍逐位一致。
...
- 固定密度时，Lasso 的数据非零数随 `m*n` 增长，应结合 `m,n,density` 控制规模。Eq QP 的 `lower_density` 同时作用于 `A` 和构造 `P` 的下部矩阵；`0<r<n` 时形成 `G @ Lambda @ G.T` 可能产生填充，默认 `r=n` 则得到对角 `P`。Random QP 的 `G @ G.T` 也可能产生填充。
